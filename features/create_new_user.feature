Feature: Create new exchange user
  An admin user
  Should be able to
  ...
  Scenario: User creates an exchange user successfully
  ...
  Scenario: User attempts to create an exchange user with a password containing their login name
    Given "tester_b_test" should not be a user
  ...
  Scenario: User attempts to create an exchange user with a password containing part of their full name
    Given "tester_b_test" should not be a user
    And I will confirm on next step
    And I follow "Create User"
    And I fill in "First Name" with "Tester_b" within "#create_new_user_container"
  ...
  Scenario: User recreates previously created exchange user after deleting him.
    ...
    And I have found the user with alias "tester_b_test" within "#application"
    And I will confirm on next step
    And I follow "Delete" within "#tester_b_test"
    Then I should see "User deleted"
    And "tester_b_test" should not be a user
    When I follow "Create User"
    And I fill in "First Name" with "Tester_b" within "#create_new_user_container"
    And I fill in "Last Name" with "Test" within "#create_new_user_container"
    And I fill in "Full Name" with "Tester Test" within "#create_new_user_container"
    And I fill in "Log On Name" with "tester_b_test" within "#create_new_user_container"
    And I fill in "User Password" with "Password2" within "#create_new_user_container"
    And I fill in "Confirm Password" with "Password2" within "#create_new_user_container"
    And I click on "#_user_ch_pwd"
    And I press "Submit" within "#create_new_user_container"
    Then I should see "User added"
    And I clean up
